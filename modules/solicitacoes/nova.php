<?php
// modules/solicitacoes/nova.php - Nova Solicitação (CORRIGIDO)
session_start();
...
// Buscar dados do funcionário (empresa_id e filial_id)
$stmt = $db->prepare("SELECT nome, matricula, empresa_id, filial_id FROM funcionarios WHERE id = :id");
$stmt->execute([':id' => $funcionario_id]);
$funcionario = $stmt->fetch();

$empresa_id = $funcionario['empresa_id'] ?? $_SESSION['empresa_id'] ?? 5;
$filial_id = $funcionario['filial_id'] ?? $_SESSION['filial_id'] ?? null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titulo)) {
        $tipo_nome = $tipos[$tipo]['nome'] ?? $tipo;
        $titulo = $tipo_nome . ' - ' . date('d/m/Y');
    }
    ...
    if (empty($errors)) {
        try {
            // CORREÇÃO: Removido 'data_solicitacao' e adicionado 'titulo'
            $stmt = $db->prepare("INSERT INTO solicitacoes 
                ...
                VALUES 
                (:funcionario_id, :empresa_id, :filial_id, :tipo, :titulo, :descricao, :data_inicio, :data_fim, 'pendente', NOW())");
            ...
            <?php else: ?>
                ...
                <form method="POST" action="" id="solicitacaoForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="titulo" placeholder="Ex: Solicitação de férias - Julho/2024">
                        <small>Opcional - Um título para sua solicitação</small>
                    </div>
