<?php
// modules/biometrico/facial.php - Cadastro Facial (CORRIGIDO)
...
async function checkModelFiles() {
    ...
    for (const file of requiredFiles) {
        try {
            ...
            updateStatusText(`❌ Arquivo ${file} não encontrado! Verifique se todos os arquivos estão na pasta assets/models.`, 'error');
            return false;
        }
    }
    return true;
...
async function loadModels() {
    ...
    try {
        ...
        const filesOk = await checkModelFiles();
        if (!filesOk) {
            throw new Error('Arquivos dos modelos não encontrados. Baixe os arquivos do repositório do face-api.js e coloque na pasta assets/models.');
        }
...
async function checkCamera() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        updateStatusText('❌ Seu navegador não suporta acesso à câmera. Use Chrome, Edge ou Firefox.', 'error');
        return false;
    }
    ...
    try {
        ...
        if (err.name === 'NotAllowedError') {
            errorMsg = '❌ Permissão negada. Clique no ícone de câmera na barra de endereço e permita o acesso.';
...
document.getElementById('btnCapturar').addEventListener('click', async () => {
    if (!window.currentDetection) {
        updateStatusText('⚠️ Nenhum rosto detectado. Posicione o rosto no círculo e aguarde a detecção.', 'warning');
    ...
    if (samplesCaptured >= totalSamples) {
        ...
        updateStatusText(`Amostra ${samplesCaptured} capturada! Continue para a próxima. (${samplesCaptured}/${totalSamples})`, 'success');
        setTimeout(() => {
            updateStatusText('✅ Rosto detectado! Clique em "Capturar Amostra" para a próxima.', 'success');
        }, 1500);
    }
});
...
async function finalizarCadastro() {
    ...
    try {
        ...
        if (result.success) {
            ...
            updateStatusText('✅ Cadastro concluído com sucesso! Redirecionando...', 'success');
