...
// Se a empresa não tiver filial, permitir cadastro sem filial
if (empty($filiais)) {
    try {
        ...
        $stmt->execute([':empresa_id' => $empresa_id]);
        $filial_padrao_id = (int) $db->lastInsertId();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($usuario_tipo === 'gestor' || $usuario_tipo === 'supervisor') && !$filial_id && !empty($usuario_filial_id)) {
        $filial_id = $usuario_filial_id;
    }
    if (($filial_id === null || $filial_id === '') && !empty($filial_padrao_id)) {
        $filial_id = $filial_padrao_id;
    }
    if (($filial_id === null || $filial_id === '') && !empty($filiais) && isset($filiais[0]['id'])) {
        $filial_id = $filiais[0]['id'];
    }
    if ($filial_id === null || $filial_id === '') {
        $errors[] = 'Filial não encontrada para esta empresa';
    }
...
<script>
// ============================================
// SISTEMA DE CAPTURA FACIAL - CADASTRO
// ============================================
