...
?>
<div class="pf-page-header"><h1><i class="fas fa-clock-rotate-left me-2 text-secondary"></i>Gerenciar batidas</h1><p class="text-muted">Consulte e corrija registros de ponto da sua equipe.</p></div>
...
<div class="card pf-table-card"><div class="card-body p-0"><div class="table-responsive"><table class="table mb-0 align-middle"><thead><tr><th>Data</th><th>Horário</th><th>Colaborador</th><th>Tipo</th><th>Filial</th><th>Origem</th><th></th></tr></thead><tbody><?php foreach($batidas as $b): ?><tr><td><?= date('d/m/Y',strtotime($b['data_hora'])) ?></td><td class="fw-semibold"><?= date('H:i',strtotime($b['data_hora'])) ?></td><td><?= htmlspecialchars($b['funcionario_nome']) ?><small class="d-block text-muted"><?= htmlspecialchars($b['matricula']) ?></small></td><td><?= htmlspecialchars($tipos[$b['tipo']] ?? $b['tipo']) ?></td><td><?= htmlspecialchars($b['filial_nome'] ?? 'Não definida') ?></td><td><?= htmlspecialchars($b['origem'] ?: 'web') ?></td><td class="text-end"><a class="btn btn-sm btn-outline-primary" href="editar_batida?id=<?= (int)$b['edit_id'] ?>"><i class="fas fa-pen me-1"></i>Editar</a></td></tr><?php endforeach; ?><?php if(!$batidas): ?><tr><td colspan="7" class="text-center text-muted py-5">Nenhuma batida encontrada neste período.</td></tr><?php endif; ?></tbody></table></div></div></div>
