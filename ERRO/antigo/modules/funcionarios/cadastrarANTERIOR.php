<?php
// modules/funcionarios/cadastrar.php - Cadastrar Novo Funcionário (COM CÂMERA)
...
function salvarFotoBase64($base64, $matricula) {
    ...
    $foto_nome = $matricula . '_' . time() . '.jpg';
    ...
    if ($img) {
        ...
        imagecopyresampled($resized, $img, 0, 0, 0, 0, $new_width, $new_height, imagesx($img), imagesy($img));
        imagejpeg($resized, $caminhoCompleto, 90);
        imagedestroy($img);
...
// Função para upload de arquivo (mantida para compatibilidade)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Foto da câmera (base64) ou upload de arquivo
    ...
    // Processar foto (prioridade para a foto da câmera)
    $foto_path = null;
    
    if ($foto_base64) {
        // Foto vinda da câmera
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                if (strpos($e->getMessage(), 'email') !== false) {
                    $error = "O e-mail informado já está cadastrado no sistema. Verifique e tente novamente.";
                } elseif (strpos($e->getMessage(), 'matricula') !== false) {
                    $error = "A matrícula informada já está cadastrada. Verifique e tente novamente.";
                } elseif (strpos($e->getMessage(), 'cpf') !== false) {
                    $error = "O CPF informado já está cadastrado. Verifique e tente novamente.";
                } else {
                    $error = "Já existe um registro com esses dados. Verifique e tente novamente.";
                }
            } else {
...
<div class="form-container">
    <div class="form-card">
        <div class="form-header">
            ...
            <p>Preencha os dados e tire uma foto para reconhecimento facial</p>
        </div>
        ...
        <form method="POST" action="" class="form-main" enctype="multipart/form-data" id="cadastroForm">
            ...
            <div class="form-section">
                ...
                <div class="foto-container">
                    ...
                    <small><i class="fas fa-info-circle"></i> A foto será usada para reconhecimento facial ao bater ponto. Tire uma foto nítida do rosto.</small>
                </div>
            </div>
            ...
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i>
                <strong>Atenção:</strong> A foto capturada será usada para validação no reconhecimento facial.
            </div>
...
<div id="cameraModal" class="camera-modal">
    <div class="camera-container">
        ...
        <div class="camera-buttons">
            <button class="btn-capturar-foto" id="capturarFoto">📸 Capturar Foto</button>
...
// Preview da foto via upload de arquivo
...
capturarBtn.addEventListener('click', function() {
    ...
    fotoPreview.innerHTML = `<img src="${fotoBase64}" alt="Preview">`;
...
document.getElementById('cadastroForm').addEventListener('submit', function(e) {
    if (!fotoCapturada && !fotoBase64Input.value) {
        if (!confirm('Nenhuma foto foi capturada. O reconhecimento facial não funcionará sem foto. Deseja continuar mesmo assim?')) {
            e.preventDefault();
            return false;
        }
    }
