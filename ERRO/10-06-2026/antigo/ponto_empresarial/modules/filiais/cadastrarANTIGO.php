<?php
// modules/filiais/cadastrar.php - Cadastrar Nova Filial (COMPLETO)
...
// Buscar empresas para o select (apenas a do usuário)
$stmt = $db->prepare("SELECT id, nome_empresa FROM empresa WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $db->prepare("SELECT id FROM filiais WHERE codigo = :codigo AND empresa_id = :empresa_id");
    ...
    if ($cnpj) {
        ...
        $check = $db->prepare("SELECT id FROM filiais WHERE cnpj = :cnpj AND empresa_id = :empresa_id");
        $check->execute([':cnpj' => $cnpj_formatado, ':empresa_id' => $empresa_id]);
